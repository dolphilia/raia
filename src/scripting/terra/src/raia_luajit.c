...
// terra_Stateはスレッドセーフではないため、libuv のミューテックスで保護
uv_mutex_t terra_mutex;

void run_terra(uv_work_t *req) {
    uv_mutex_lock(&terra_mutex);
    ...
    // if (luaL_dofile(L, "main.t") != LUA_OK) { // Luaスクリプトをロード・実行
    ...
    //Terraの状態をLuaに初期化
    terra_init(L);
    if (terra_dofile(L, "main.t")) {
        exit(1);
    }

    lua_close(L); // Lua環境を閉じる

    uv_mutex_unlock(&terra_mutex);
}

// タスク完了後に呼ばれるコールバック
void after_run_terra(uv_work_t *req, int status) {
    if (status == 0) {
        printf("Task completed successfully.\n");
    } else {
        fprintf(stderr, "Task completed with error: %d\n", status);
    }
}
...
RAIA_API const char *init(int argc, char *argv[]) {
    ...
    // ミューテックスの初期化
    uv_mutex_init(&terra_mutex);

    // タスクをキューに追加
    uv_work_t req1, req2;
    uv_queue_work(loop, &req1, run_terra, after_run_terra);
    //uv_queue_work(loop, &req2, run_terra, after_run_terra); // 必要に応じてスレッドを作成

    // イベントループの開始
    uv_run(loop, UV_RUN_DEFAULT);

    // ミューテックスの破棄
    uv_mutex_destroy(&terra_mutex);
    return 0;
}
