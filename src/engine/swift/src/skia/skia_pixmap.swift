extension Skia {
    protocol SkPixmapProtocol {
        var pointer: Skia.SkPixmapMutablePointer? { get set }
        var handle: sk_pixmap_t { get set }
        // deinit // void SkPixmap_delete(void *pixmap); // (SkPixmap *pixmap)
        init(pointer: Skia.SkPixmapMutablePointer?, handle: sk_pixmap_t)
        init () // void *SkPixmap_new(); // () -> SkPixmap *
        init(info: Skia.ImageInfo, addr: UnsafeRawPointer?, rowBytes: UInt) // void *SkPixmap_new_2(const void *info, const void *addr, unsigned long rowBytes); // (const SkImageInfo *info, const void *addr, size_t rowBytes) -> SkPixmap *
        // Methods
        func reset() // void SkPixmap_reset(void *pixmap); // (SkPixmap *pixmap)
        func reset(info: ImageInfo, addr: UnsafeRawPointer?, rowBytes: UInt) // void SkPixmap_reset_2(void *pixmap, const void *info, const void *addr, unsigned long rowBytes); // (SkPixmap *pixmap, const SkImageInfo *info, const void *addr, size_t rowBytes)
        func reset(mask: Mask) -> Bool // bool SkPixmap_reset_3(void *pixmap, const void *mask); // (SkPixmap *pixmap, const SkMask *mask) -> bool
        func setColorSpace(colorSpace: ColorSpace) // void SkPixmap_setColorSpace(void *pixmap, int color_space); // (SkPixmap *pixmap, sk_color_space_t color_space)
        func extractSubset(subset: SkPixmap, area: IRect) -> Bool // bool SkPixmap_extractSubset(void *pixmap, void *subset, const void *area); // (SkPixmap *pixmap, SkPixmap *subset, const SkIRect *area) -> bool
        func info() -> ImageInfo // const void *SkPixmap_info(void *pixmap); // (SkPixmap *pixmap) -> const SkImageInfo *
        ...
        func dimensions() -> ISize // int SkPixmap_dimensions(void *pixmap); // (SkPixmap *pixmap) -> sk_i_size_t
        ...
        func colorSpace() -> ColorSpace // void *SkPixmap_colorSpace(void *pixmap); // (SkPixmap *pixmap) -> SkColorSpace *
        func refColorSpace() -> ColorSpace // int SkPixmap_refColorSpace(void *pixmap); // (SkPixmap *pixmap) -> sk_color_space_t
        func isOpaque() -> Bool // bool SkPixmap_isOpaque(void *pixmap); // (SkPixmap *pixmap) -> bool
        func bounds() -> IRect // int SkPixmap_bounds(void *pixmap); // (SkPixmap *pixmap) -> sk_i_rect_t
        ...
        func getColor(x: Int, y: Int) -> ColorARGB8888 // unsigned int SkPixmap_getColor(void *pixmap, int x, int y); // (SkPixmap *pixmap, int x, int y) -> SkColor
        func getColor4f(x: Int, y: Int) -> Color4f // int SkPixmap_getColor4f(void *pixmap, int x, int y); // (SkPixmap *pixmap, int x, int y) -> sk_color_4f_t
        ...
        func readPixels(dstInfo: ImageInfo, dstPixels: UnsafeMutableRawPointer?, dstRowBytes: UInt) -> Bool // bool SkPixmap_readPixels(void *pixmap, const void *dstInfo, void *dstPixels, unsigned long dstRowBytes); // (SkPixmap *pixmap, const SkImageInfo *dstInfo, void *dstPixels, size_t dstRowBytes) -> bool
        func readPixels(dstInfo: ImageInfo, dstPixels: UnsafeMutableRawPointer?, dstRowBytes: UInt, srcX: Int, srcY: Int) -> Bool // bool SkPixmap_readPixels_2(void *pixmap, const void *dstInfo, void *dstPixels, unsigned long dstRowBytes, int srcX, int srcY); // (SkPixmap *pixmap, const SkImageInfo *dstInfo, void *dstPixels, size_t dstRowBytes, int srcX, int srcY) -> bool
        ...
        func erase(color: ColorARGB8888, subset: IRect) -> Bool // bool SkPixmap_erase(void *pixmap, unsigned int color, const void *subset); // (SkPixmap *pixmap, SkColor color, const SkIRect *subset) -> bool
        func erase(color: ColorARGB8888) -> Bool // bool SkPixmap_erase_2(void *pixmap, unsigned int color); // (SkPixmap *pixmap, SkColor color) -> bool
        func erase(color: Color4f, subset: IRect) -> Bool // bool SkPixmap_erase_3(void *pixmap, const void *color, const void *subset); // (SkPixmap *pixmap, const SkColor4f *color, const SkIRect *subset) -> bool
    }

    class SkPixmap : SkPixmapProtocol {
        public var pointer: Skia.SkPixmapMutablePointer?
        ...
        required init(pointer: Skia.SkPixmapMutablePointer?, handle: sk_pixmap_t) {
        ...
        required init(info: Skia.ImageInfo, addr: UnsafeRawPointer?, rowBytes: UInt) {
        ...
        func extractSubset(subset: SkPixmap, area: IRect) -> Bool {
