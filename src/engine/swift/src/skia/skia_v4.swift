extension Skia {
    protocol SKV4Protocol {
        var pointer: SkV4MutablePointer? { get set }
        var handle: sk_v4_t { get set }
        // deinit // void SkV4_delete(void *v4); // (SkV4 *v4)
        init(pointer: SkV4MutablePointer?, handle: sk_v4_t)
        // Static Methods
        static func Dot(a: SkV4, b: SkV4) -> Float // float SkV4_Dot(const void *a, const void *b); // (const SkV4 *a, const SkV4 *b) -> SkScalar
        static func Normalize(v: SkV4) -> SkV4 // int SkV4_Normalize(const void *v); // (const SkV4 *v) -> sk_v4_t
        // Methods
        ...
        func dot(v: SkV4) -> Float // float SkV4_dot(void *v4, const void *v); // (SkV4 *v4, const SkV4 *v) -> SkScalar
        func normalize() -> SkV4 // int SkV4_normalize(void *v4); // (SkV4 *v4) -> sk_v4_t
    ...
    class SkV4 : SKV4Protocol {
        public var pointer: SkV4MutablePointer?
        ...
        required init(pointer: SkV4MutablePointer?, handle: sk_v4_t) {
        ...
        static func Dot(a: SkV4, b: SkV4) -> Float {
            return SkV4_Dot(a.pointer, b.pointer)
        }

        static func Normalize(v: SkV4) -> SkV4 {
