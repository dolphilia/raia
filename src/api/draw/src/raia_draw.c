...
static void _draw_noise_cpu(uint8_t *pixels, int width, int height) {
    for (int i = 0; i < width * height * 3; i++) {
        pixels[i] = rand_xor128(0, 254);
    }
}

static void _draw_noise_gpu(uint8_t *pixels, int width, int height) {
...
RAIA_API const char * raia_draw_noise_cpu(const char *s) {
    ...
    _draw_noise_cpu(pixels, width, height);
    joint_free(joint);
    return NULL;
}

RAIA_API const char * raia_draw_noise_gpu(const char *s) {
    joint_t *joint = joint_init_with_str(s);
    uint8_t *pixels = (uint8_t *)(uintptr_t)joint_get_in_uint(joint, "pixels");
    int width = joint_get_in_int(joint, "width");
    int height = joint_get_in_int(joint, "height");
    _draw_noise_gpu(pixels, width, height);
    joint_free(joint);
    return NULL;
}

RAIA_API const char * raia_draw_point(const char *s) {
    joint_t *joint = joint_init_with_str(s);
    uint8_t *pixels = (uint8_t *)(uintptr_t)joint_get_in_uint(joint, "pixels");
    raia_point_t point = {
