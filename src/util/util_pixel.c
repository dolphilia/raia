...
void set_pixel_rgb(uint8_t *pixel_data, int32_t point_x, int32_t point_y, uint8_t color_red, uint8_t color_green, uint8_t color_blue, int32_t canvas_size_width, int32_t canvas_size_height, int32_t samples_per_pixel) {
    ...
    int index = point_y * canvas_size_width * samples_per_pixel + point_x * samples_per_pixel;
...
void set_pixel_rgb_protect_alpha(uint8_t *pixel_data, int32_t point_x, int32_t point_y, uint8_t color_red, uint8_t color_green, uint8_t color_blue, int32_t canvas_size_width, int32_t canvas_size_height, int32_t protect_alpha, int32_t samples_per_pixel) {
    ...
    color_t color = get_pixel_color(pixel_data, point_x, point_y, canvas_size_width, canvas_size_height, samples_per_pixel);
    ...
    if (deltax >= deltay) {
        ...
        for (int i = 0; i <= deltax; ++i) {
            if (x<0 || x>canvas_size_width || y<0 || y>canvas_size_height) { //描画範囲を超える場合
                ...
                set_pixel_rgb(pixel_data, x, y, color_red, color_green, color_blue, canvas_size_width, canvas_size_height, samples_per_pixel);
            }
            x += xstep;
            error += 2 * deltay;
            if (error >= 0) {
    ...
    for (int y = 0; y < diffy; y++) {
        for (int x = 0; x < diffx; x++) {
            set_pixel_rgb(pixel_data,
                ...
                canvas_size_width, canvas_size_height, samples_per_pixel);
        }
    }
}
    ...
    for (int iy = start_point_y; iy < end_point_y + heightRadius; iy++) {
        for (int ix = start_point_x; ix < end_point_x + widthRadius; ix++) {
            ...
            if (x * x + y * y < widthRadius * widthRadius) {
                set_pixel_rgb(pixel_data, ix, iy, color_red, color_green, color_blue, canvas_size_width, canvas_size_height, samples_per_pixel);
            }
        }
    }
}
...
//                    set_pixel_rgba_protect_alpha_fast(pixel_data_dest, x + cx, y + cy, color.red, color.green, color.blue, color.alpha, canvas_dest_size_width, canvas_dest_size_height, pixel_data_source[((x + copy_point_x) * 4 + 3) + y * canvas_dest_size_width * 4], set_pixel_index, samples_per_pixel);
//                }
//            }
//            break;
//        }
