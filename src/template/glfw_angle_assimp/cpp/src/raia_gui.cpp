...
void LoadModel(const std::string& path, std::vector<float>& vertices, std::vector<unsigned int>& indices) {
    ...
    // メッシュデータの処理
    aiMesh* mesh = scene->mMeshes[0]; // 最初のメッシュを使用
...
int main(int argc, char* argv[]) {
    ...
    std::vector<float> vertices_assimp;
    std::vector<unsigned int> indices_assimp;

    // モデルを読み込む
    LoadModel("teapot.obj", vertices_assimp, indices_assimp);
