...
static std::set<int> static_sk_shaper_script_run_iterator_available_keys;
static std::map<int , std::unique_ptr<SkShaper::ScriptRunIterator>> static_sk_shaper_script_run_iterator;
static int static_sk_shaper_script_run_iterator_index = 0;

int static_sk_shaper_script_run_iterator_make(std::unique_ptr<SkShaper::ScriptRunIterator> value) {
    int key;
    if (!static_sk_shaper_script_run_iterator_available_keys.empty()) {
        auto it = static_sk_shaper_script_run_iterator_available_keys.begin();
        key = *it;
        static_sk_shaper_script_run_iterator_available_keys.erase(it);
    } else {
        key = static_sk_shaper_script_run_iterator_index++;
    }
...
void static_sk_shaper_script_run_iterator_delete(int key) {
    ...
    static_sk_shaper_script_run_iterator_available_keys.insert(key);
}
