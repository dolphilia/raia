...
int static_sk_shaper_script_run_iterator_make(std::unique_ptr<SkShaper::ScriptRunIterator> value) {
    static_sk_shaper_script_run_iterator[static_sk_shaper_script_run_iterator_index] = std::move(value);
    static_sk_shaper_script_run_iterator_index++;
    return static_sk_shaper_script_run_iterator_index - 1;
}
