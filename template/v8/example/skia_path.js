...
// パスを描画する
var path = skia.SkPath_new();
var cx = 800 / 2;
var cy = 600 / 2;
var outerRadius = 100;
var innerRadius  = 50;
var numPoints = 5;
var kAngleStep = (2 * Math.PI) / numPoints;
var kHalfAngleStep = kAngleStep / 2;
skia.SkPath_moveTo(path, cx + outerRadius, cy);
for (var i = 1; i < numPoints * 2; ++i) {
    var angle = kAngleStep * i;
    var radius = (i % 2 == 0) ? outerRadius : innerRadius;
    skia.SkPath_lineTo(path, cx + radius * Math.cos(angle), cy + radius * Math.sin(angle));
}
skia.SkPath_close(path);
var paint = skia.SkPaint_new();
skia.SkPaint_setColor(paint, 0xffffff00);
skia.SkPaint_setAntiAlias(paint, 1);
skia.SkCanvas_drawPath(canvas, path, paint);
