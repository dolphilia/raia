...
-- テーブルに特定のキーが存在するかを一括で判定する関数
local function keyExists(table, ...)
    -- 可変長引数をループ処理
    for _, key in ipairs({...}) do
        if table[key] == nil then
            -- 存在しないキーがあればエラーを出力して終了
            error("Error: Key '" .. key .. "' does not exist in the table.")
        end
    end
    -- すべてのキーが存在すればtrueを返す
    return true
...
-- テーブルに特定のキーが存在するかつ、型が一致しているかをチェックする関数
local function checkArgs(args, keyTypeMap)
    for key, expectedType in pairs(keyTypeMap) do
        -- キーが存在するかチェック
        if args[key] == nil then
            error("Error: Key '" .. key .. "' does not exist in the table.")
        end
        -- 型が一致するかチェック
        local actualType = raia.lua.type(args[key])
        if actualType ~= expectedType then
            error("Error: Key '" .. key .. "' expected to be of type '" 
                .. expectedType .. "', but found type '" .. actualType .. "'.")
        end
    end
    -- すべてのチェックが通った場合
    return true
...
local function checkNil(args)
    for key, value in pairs(args) do
        print("a")
        if value == nil then
            print("Key " .. key .. " is nil")
            os.exit()
        end
    end
end

local function testFn(args)
    --keyExists(args, {"a", "b", "c"}) -- キーが存在するかチェックする
    checkArgs(args, {a = Number, b = Number, c = Number}) -- キーが存在するかチェックする
end
