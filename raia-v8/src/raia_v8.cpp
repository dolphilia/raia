...
void raia_io_load_script(const fci_val_t &args) {
    ...
    //auto filename_extension = append_extension(filename.c_str(), config.ts_mode);
    //auto directories = get_directories(config.root_dir);
...
void raia_io_save_script(const fci_val_t &args) {
    ...
    if (!outFile) {
        ...
        return;  // ファイルが開けない場合、エラーとしてfalseを返す
    }
...
void raia_require(const fci_val_t& args) {
    ...
    //auto code = load_module_script(*str, config.root_dir,false);
...
ml_mod_t import_static_callback(l_ctx_t context, l_str_t specifier, l_mod_t referrer) {
    ...
    //return import_load(code, *str, context);
}
...
ml_promise_t import_dynamic_callback(l_ctx_t context, l_som_t referrer, l_str_t specifier) {
    ...
    //auto code = load_module_script(*name, config.root_dir,false);
...
int raia_v8_main(int argc, char* argv[]) {
...

    {
        ...
        //auto value = result->ToString(context).ToLocalChecked();
        //v8::String::Utf8Value utf8(isolate, value);
        //std::string stdStr(*utf8);

//        //c++ から v8の関数を呼び出す
//        // 関数の名前を作成
//        v8::Local<v8::String> funcName = v8::String::NewFromUtf8(isolate, "typescript_transpile").ToLocalChecked();
//        // グローバルオブジェクトから関数を取得
//        v8::Local<v8::Value> jsFuncVal = context->Global()->Get(context, funcName).ToLocalChecked();
//
...
//            // 関数の引数を作成
//            const int fn_argc = 1;
//            v8::Local<v8::Value> fn_argv[fn_argc] = { v8::String::NewFromUtf8(isolate, "my argument").ToLocalChecked() };
//
//            // 関数を呼び出す
//            v8::Local<v8::Value> result = jsFunc->Call(context, context->Global(), argc, fn_argv).ToLocalChecked();
//        }
        ...
        if (get_raia_config().ts_mode) {
            // // var result = ts.transpileModule(source, { compilerOptions: { target: ts.ScriptTarget.ES2015, module: ts.ModuleKind.ES2015 }});
        ...
        //
//        v8::Local<v8::String> obj_name = v8::String::NewFromUtf8(isolate, "typescript_compiler").ToLocalChecked();
//        v8::Local<v8::Value> jsObjVal = context->Global()->Get(context, obj_name).ToLocalChecked();
//
//        if (jsObjVal->IsObject()) {
//            v8::Local<v8::Object> jsObj = v8::Local<v8::Object>::Cast(jsObjVal);
//
//            v8::Local<v8::String> funcName = v8::String::NewFromUtf8(isolate, "transpileModule").ToLocalChecked();
//            v8::Local<v8::Value> jsFuncVal = jsObj->Get(context, funcName).ToLocalChecked();
//
//            if (jsFuncVal->IsFunction()) {
//                v8::Local<v8::Function> jsFunc = v8::Local<v8::Function>::Cast(jsFuncVal);
//
...
//
//                // 関数を呼び出す
//                v8::Local<v8::Value> result = jsFunc->Call(context, jsObj, argc, fn_argv).ToLocalChecked();
//            }
//        }
    }
